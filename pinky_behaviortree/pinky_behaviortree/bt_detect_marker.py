    ...
    def image_callback(self, msg):
        try:
            ...
            if ids is not None and len(ids) > 0:
                ...
                self.detected_marker_id = ids[0][0]  # 첫 번째 감지된 마커의 ID
                self.marker_tvec = tvecs[0][0]  # 첫 번째 감지된 마커의 위치 벡터
    ...
    def update(self):
        ...
        if elapsed_time < self.detection_time:
            if self.marker_detected:
                ...
                # 디버깅을 위한 로그 추가
                self.node.get_logger().info(f'Stored in blackboard - ID: {self.detected_marker_id}, tvec: {self.marker_tvec}')
